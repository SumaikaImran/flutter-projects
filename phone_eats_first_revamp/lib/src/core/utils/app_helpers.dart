...
class AppHelpers {
  ...
  static String getContentTypeIcon(String key) {
    String icon = AppAssets.imageIcn;
    if (key.contains('pdf')) {
      icon = AppAssets.pptIcn;
    } else if (key.contains('pdf')) {
      icon = AppAssets.pdfIcn;
    } else if (key.contains('xls')) {
      icon = AppAssets.xlsIcn;
    } else if (key.contains('mp4') || key.contains('mov')) {
      icon = AppAssets.mp4Icn;
    } else {
      icon = AppAssets.imageIcn;
    }
    return icon;
  }
...
// add method to share multiple files via share_plus package
  ...
  static Future<XFile> compressXFileWithImagePackage(XFile xfile,
